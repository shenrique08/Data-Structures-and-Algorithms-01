...
int listas_iguais(Lista *lista1, Lista *lista2) {
    ...
    // base case: one list is empty and the other is not
    if (lista1 == NULL || lista2 == NULL) {
        printf("\nAs listas sao DIFERENTES!!!\n\n");
        return 0;
    }
    // recursive case: both lists have at least one element
    if (aux1->valor != aux2->valor) {
        ...
        aux1 = aux1->prox;
        aux2 = aux2->prox;
        return listas_iguais((aux1 == NULL) ? NULL : &aux1, (aux2 == NULL) ? NULL : &aux2);
    }
}
...
int recursive_sum(Lista *lista)
{
    ...
    if (node->prox == NULL)
        ...
        return (node->valor) + recursive_sum(&(node->prox));
    }
}
...
void inverse_list(Lista *lista)
{
    ...
    printf("\n\nValores na lista (em ordem inversa):\n\n[");
    No *aux = prev;
    while (aux != NULL) {
        printf(" %d ", aux->valor);
        aux = aux->prox;
...
int remove_num(Lista *lista, int num)
{
    // Verifica se a lista existe
    if (lista == NULL) {
        fprintf(stderr, "\nA lista nao existe!!!\n\n");
...
int retira_repeticoes(Lista *lista)
{
    // Verifica se a lista existe
    if (lista == NULL) {
        fprintf(stderr, "\nA lista nao existe!!!\n\n");
    ...
    if (qtd_dado_numero(lista, (*lista)->valor) > 1) {
        found_same = 1;
        remove_num(lista, (*lista)->valor);
    }

    if (retira_repeticoes(&no_aux) > 0) {
        found_same = 1;
    }
